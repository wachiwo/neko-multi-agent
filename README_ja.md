...
```
      あなた（ご主人様）
           ...
           ▼ 命令を出す
    ┌─────────────┐
    │   OYABUN     │  ← 親分猫：命令を受け取り、頭猫に委譲
    │  (親分猫)    │
    └──────┬──────┘
           │ YAMLファイル + tmux
    ┌──────▼──────┐
    │   KASHIRA    │  ← 頭猫：タスクを作業猫(犬)に分配
    │  (頭猫)    │
    └──────┬──────┘
           │
    ┌──┬──┼──┬──┐
...
| 頭猫（Kashira） | 管理者 - タスクを分配・コードレビュー | 1 |
| 作業猫(犬) | ワーカー - 並列でタスク実行 | 4 |

tmuxセッションが作成されます：
- `oyabun` - ここに接続してコマンドを出す
- `multiagent` - 頭猫 + 作業猫(犬)4体が稼働（5ペイン）
...
作業猫(犬)はエラー時に最大3回自動リトライ（毎回アプローチを変える）。3回失敗したら頭猫が別のワーカーに再割当。

### 7. コードレビュー

頭猫がコード成果物をレビュー（構文・セキュリティ・パフォーマンス・可読性）。問題があればフィードバック付きで修正指示。
...
### なぜ階層構造（親分猫→頭猫→作業猫）なのか

1. **即時応答**: 親分猫は即座に委譲してご主人様に制御を返す
2. **並列実行**: 頭猫が複数の作業猫(犬)に同時にタスクを分配
3. **関心の分離**: 親分猫が「何を」、頭猫が「誰に」を決める
...
### なぜ dashboard.md は頭猫のみが更新するのか

1. **単一更新者**: 競合を防ぐため、更新責任者を1人に限定
2. **情報集約**: 頭猫は全作業猫(犬)の報告を受ける立場なので全体像を把握
...
|
+-- queue/                    # 通信ファイル
|   +-- oyabun_to_kashira.yaml # 親分猫から頭猫へのコマンド
|   +-- tasks/                # 各ワーカーのタスクファイル
|   +-- reports/              # ワーカーレポート
|
