#!/bin/bash
# =^._.^= neko-multi-agent おさんぽスクリプト（毎日の起動用）
# Daily Deployment Script for Neko Multi-Agent Orchestration System
#
# 使用方法:
#   ./osanpo.sh           # 全エージェント起動（通常）
#   ./osanpo.sh -s        # セットアップのみ（Claude起動なし）
#   ./osanpo.sh -h        # ヘルプ表示
...
while [[ $# -gt 0 ]]; do
    case $1 in
        ...
        -h|--help)
            echo ""
            echo "=^._.^= neko-multi-agent おさんぽスクリプト"
            echo ""
            echo "使用方法: ./osanpo.sh [オプション]"
            echo ""
            echo "オプション:"
            ...
            echo ""
            echo "例:"
            echo "  ./osanpo.sh      # 全エージェント起動（通常のおさんぽ）"
            echo "  ./osanpo.sh -s   # セットアップのみ（手動でClaude起動）"
            echo "  ./osanpo.sh -t   # 全エージェント起動 + ターミナルタブ展開"
            echo ""
            echo "エイリアス:"
            echo "  csst  → cd $(pwd) && ./osanpo.sh"
...
show_neko_banner() {
    ...
    echo -e "\033[1;35m╠══════════════════════════════════════════════════════════════════════════════════╣\033[0m"
    echo -e "\033[1;35m║\033[0m       \033[1;37mおさんぽするにゃ〜！！！\033[0m    \033[1;33m=^._.^=\033[0m    \033[1;36mみんな集合にゃ！\033[0m                  \033[1;35m║\033[0m"
    echo -e "\033[1;35m╚══════════════════════════════════════════════════════════════════════════════════╝\033[0m"
    echo ""
