#!/usr/bin/env python3
"""subtask_184_b2b_005_w2 self-verify screenshot.

005_受注区分別実績表.html の B2b 本格改修後、
...
TARGET = Path(
    "/mnt/i/仕事/001_ディムコ/001_ソース/001_モック/dimco-prototype/new/005_受注区分別実績表.html"
)
OUTDIR = Path(
    "/mnt/c/tools/neko-multi-agent/outputs/dimco-prototype/"
    "cmd_184_b2b/phase1_w2"
)
ZOOMS = [1.0, 1.5, 2.0]
            ...
            probe = page.evaluate(
                """() => {
                    ...
                    return {
                        ...
                        btn_exists: !!btn,
                        btn_parent_class: btn ? btn.parentElement.className || btn.parentElement.getAttribute('style') : null,
                        ...
                        tab_count: document.querySelectorAll('.tab-button').length,
                    };
                }"""
            )
            print(f"--- zoom {pct}% ---")
