#!/usr/bin/env python3
"""subtask_184_b2b_006_w3 self-verify screenshot.

006_受注売上入金管理.html の B2b Phase 1 改修後、
zoom 100%/150%/200% の3段階で form-row/form-field canonical 準拠と overflow を確認。
"""
...
TARGET = Path(
    "/mnt/i/仕事/001_ディムコ/001_ソース/001_モック/dimco-prototype/new/006_受注売上入金管理.html"
)
OUTDIR = Path(
    "/mnt/c/tools/neko-multi-agent/outputs/dimco-prototype/"
    "cmd_184_b2b/phase1_w3"
)
ZOOMS = [1.0, 1.5, 2.0]
            ...
            probe = page.evaluate(
                """() => {
                    ...
                    return {
                        ...
                        form_row_width_px: rowRect ? Math.round(rowRect.width) : null,
                    };
                }"""
            )
