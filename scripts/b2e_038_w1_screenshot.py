#!/usr/bin/env python3
"""subtask_184_b2e_038_w1 self-verify screenshot."""
...
TARGET = Path(
    "/mnt/i/仕事/001_ディムコ/001_ソース/001_モック/dimco-prototype/new/038_未出荷買掛一覧表出力指示.html"
)
OUTDIR = Path(
    "/mnt/c/tools/neko-multi-agent/outputs/dimco-prototype/cmd_184_b2e/phase1_w1"
)
ZOOMS = [1.0, 1.5, 2.0]
...
def main():
    ...
    with sync_playwright() as p:
        ...
        for zoom in ZOOMS:
            ...
            pct = int(zoom * 100)
            out = OUTDIR / f"038_zoom{pct}.png"
            page.screenshot(path=str(out), full_page=True)
            probe = page.evaluate(
                """() => {
                    const row = document.querySelector('.form-row');
                    const fields = document.querySelectorAll('.form-field');
                    const inlineGroups = document.querySelectorAll('.form-field-inline');
                    ...
                    return {
                        ...
                        form_field_inline_count: inlineGroups.length,
                        form_field_inline_display: inlineGroups[0] ? getComputedStyle(inlineGroups[0]).display : null,
