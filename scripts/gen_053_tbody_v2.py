#!/usr/bin/env python3
"""Generate tbody for 053: parent ▷ opens SOLD TO, SOLD TO row has ▷ to open INVOICE/明細."""
...
for i, r in enumerate(rows):
    ...
    # Parent row: 1 expand button → opens SOLD TO detail
    lines.append(
    ...
    # Detail Level 1: SOLD TO header (detail-target-1)
    lines.append(
        ...
        f'<td>COUNTRY</td><td>TEL</td><td>FAX</td><td class="detail-empty"></td></tr>'
    )
    # Detail Level 1: SOLD TO data with ▷ button on left for level 2
    lines.append(
    ...
    # Detail Level 2: INVOICE / 明細 header (detail-target-2)
    lines.append(
        ...
        f'<td>QUANTITY</td><td>UNIT PRICE</td><td>AMOUNT</td><td class="detail-empty"></td></tr>'
    )
    # Detail Level 2: INVOICE / 明細 data
    lines.append(
        ...
        f'<td class="text-right">{qty}</td><td class="text-right">{uprice}</td><td class="text-right">{amt2}</td><td class="detail-empty"></td></tr>'
    )
    lines.append('')
