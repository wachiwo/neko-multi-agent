#!/usr/bin/env python3
"""Generate tbody v3: indent detail levels.
Level 1 (SOLD TO): 1 empty col on left (for ▷ btn in data row)
Level 2 (明細):    2 empty cols on left, 1 empty col on right -> more indented
"""
...
# Parent row: 13 cols = [▷][操作][書類種別][商伝番号][得意先CD][得意先名称][出荷日付][P.O.NO][REF.NO][直送先][AMOUNT][営業担当者][PACKING]
# Level1 header: indent 1 col → [empty(1)] + 11 data cols + [empty(1)] = 13
# Level1 data:   [▷btn(1)] + 11 data cols + [empty(1)] = 13
# Level2 header: indent 2 cols → [empty(2)] + 10 data cols + [empty(1)] = 13
# Level2 data:   [empty(2)] + 10 data cols + [empty(1)] = 13
...
for i, r in enumerate(rows):
    ...
    # Level 1 header: 1 empty left + 11 data cols + 1 empty right = 13
    # 11 data: SOLD TO1, SOLD TO2(colspan2), SOLD TO3, ADDRESS1(colspan2), ADDRESS2, ADDRESS3, COUNTRY, TEL, FAX
    # That's actually: 1+2+1+2+1+1+1+1+1 = 11 logical but with colspans: SOLD TO1(1) + SOLD TO2(2) + SOLD TO3(1) + ADDRESS1(2) + ADDRESS2(1) + ADDRESS3(1) + COUNTRY(1) + TEL(1) + FAX(1) = 11 physical cols
    # Total: 1 + 11 + 1 = 13 ✓
    lines.append(
        f'                    <tr class="detail-header detail-target-1">'
        f'<td class="detail-empty"></td>'
        f'<td>SOLD TO1</td><td colspan="2">SOLD TO2</td><td>SOLD TO3</td>'
        f'<td colspan="2">ADDRESS1</td><td>ADDRESS2</td><td>ADDRESS3</td>'
        f'<td>COUNTRY</td><td>TEL</td><td>FAX</td>'
        f'<td class="detail-empty"></td></tr>'
    )

    # Level 1 data: ▷ button left + 11 data cols + 1 empty right = 13
    lines.append(
        f'                    <tr class="detail-row detail-target-1">'
        f'<td><button class="expand-btn expand-btn-sub" onclick="toggleDetail2(this)">▷</button></td>'
    ...
    # Level 2 header: 2 empty left + 10 data cols + 1 empty right = 13
    # 10 data: INVOICE NO(1) + TERMS(2) + TERMS OF PAYMENT(2) + NO(1) + DESCRIPTION(1) + QUANTITY(1) + UNIT PRICE(1) + AMOUNT(1) = 10 physical cols
    lines.append(
        f'                    <tr class="detail-header detail-target-2">'
        f'<td class="detail-empty"></td><td class="detail-empty"></td>'
        f'<td>INVOICE NO</td><td colspan="2">TERMS</td><td colspan="2">TERMS OF PAYMENT</td>'
        f'<td>NO</td><td>DESCRIPTION</td>'
        f'<td>QUANTITY</td><td>UNIT PRICE</td><td>AMOUNT</td>'
        f'<td class="detail-empty"></td></tr>'
    )

    # Level 2 data: 2 empty left + 10 data cols + 1 empty right = 13
    lines.append(
        ...
        f'<td class="text-right">{qty}</td><td class="text-right">{uprice}</td><td class="text-right">{amt2}</td>'
        f'<td class="detail-empty"></td></tr>'
    )
    lines.append('')
