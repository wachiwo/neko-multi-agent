#!/usr/bin/env python3
"""Generate tbody v4: visually indented detail levels.
Parent:  13 cols = [▷][操作][書類種別][商伝番号][得意先CD][得意先名称][出荷日付][P.O.NO][REF.NO][直送先][AMOUNT][営業担当者][PACKING]
Level 1: 13 cols = [empty][empty][data x 10][empty]  (2 empty left = 1段ずれ)
Level 1 data:      [empty][▷btn][data x 10][empty]
Level 2: 13 cols = [empty][empty][empty][data x 9][empty]  (3 empty left = 2段ずれ)
"""
...
for i, r in enumerate(rows):
    ...
    # Level 1 header: [empty][empty] + 10 data cols + [empty] = 13
    # SOLD TO1(1)+SOLD TO2(cs2)+SOLD TO3(1)+ADDRESS1(cs2)+ADDRESS2(1)+ADDRESS3(1)+COUNTRY(1)+TEL(1) = 10 physical
    # + empty right = 11 → total with 2 empty left = 13
    lines.append(
    ...
    # Level 1 data: [empty][▷btn] + 10 data cols + [empty] = 13
    lines.append(
    ...
    # Level 2 header: [empty][empty][empty] + 9 data cols + [empty] = 13
    # INVOICE NO(1)+TERMS(cs2)+TERMS OF PAYMENT(1)+NO(1)+DESCRIPTION(1)+QUANTITY(1)+UNIT PRICE(1)+AMOUNT(1) = 9 physical
    # + empty right = 10 → total with 3 empty left = 13
    lines.append(
        f'                    <tr class="detail-header detail-target-2">'
        f'<td class="detail-empty"></td><td class="detail-empty"></td><td class="detail-empty"></td>'
        f'<td>INVOICE NO</td><td colspan="2">TERMS</td><td>TERMS OF PAYMENT</td>'
        f'<td>NO</td><td>DESCRIPTION</td>'
        f'<td>QUANTITY</td><td>UNIT PRICE</td><td>AMOUNT</td>'
        f'<td class="detail-empty"></td></tr>'
    )

    # Level 2 data: [empty][empty][empty] + 9 data cols + [empty] = 13
    lines.append(
        f'                    <tr class="detail-row detail-target-2">'
        f'<td class="detail-empty"></td><td class="detail-empty"></td><td class="detail-empty"></td>'
        f'<td>{inv_no}</td><td colspan="2">{terms}</td><td>{payment}</td>'
