#!/usr/bin/env python3
"""Generate tbody v5: ▷ button inside SOLD TO1 cell, not in separate column.
Parent:       13 cols = [▷][操作][書類種別][商伝番号][得意先CD][得意先名称][出荷日付][P.O.NO][REF.NO][直送先][AMOUNT][営業担当者][PACKING]
Level1 hdr:   13 cols = [empty cs2] + SOLD TO1 + SOLD TO2(cs2) + SOLD TO3 + ADDRESS1(cs2) + ADDRESS2 + ADDRESS3 + COUNTRY + TEL + FAX
Level1 data:  13 cols = [empty cs2] + [▷+SOLD TO1] + SOLD TO2(cs2) + SOLD TO3 + ADDRESS1(cs2) + ADDRESS2 + ADDRESS3 + COUNTRY + TEL + FAX
Level2 hdr:   13 cols = [empty cs3] + INVOICE NO + TERMS(cs2) + T.O.PAYMENT + NO + DESCRIPTION + QTY + UPRICE + AMOUNT + [empty]
Level2 data:  13 cols = [empty cs3] + data... + [empty]
"""
...
for i, r in enumerate(rows):
    ...
    # Level 1 header: [empty cs2] + 11 data = 13
    lines.append(
        f'                    <tr class="detail-header detail-target-1">'
        f'<td colspan="2" class="detail-empty"></td>'
    ...
    # Level 1 data: [empty cs2] + [▷ inside SOLD TO1 cell] + 10 data = 13
    # ▷ button is INSIDE the SOLD TO1 td, not a separate cell
    lines.append(
        f'                    <tr class="detail-row detail-target-1">'
        f'<td colspan="2" class="detail-empty"></td>'
        f'<td class="text-left"><button class="expand-btn expand-btn-sub" onclick="toggleDetail2(this)">▷</button> {sold1}</td>'
        f'<td colspan="2" class="text-left">{sold2}</td><td class="text-left">{sold3}</td>'
        f'<td colspan="2" class="text-left">{addr1}</td><td class="text-left">{addr2}</td><td class="text-left">{addr3}</td>'
        f'<td>{country}</td><td>{tel}</td><td>{fax}</td></tr>'
    )

    # Level 2 header: [empty cs3] + 9 data + [empty] = 13
    lines.append(
    ...
    # Level 2 data: [empty cs3] + 9 data + [empty] = 13
    lines.append(
