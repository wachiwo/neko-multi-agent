#!/usr/bin/env python3
"""Generate tbody v7: nested table approach (like 016_受注一覧).
Parent row → colspan child-table-container → child table (SOLD TO)
Child row → colspan grandchild-table-container → grandchild table (INVOICE/明細)
"""
...
for i, r in enumerate(rows):
    ...
    # Child row: colspan=13 → child-table (SOLD TO)
    ...
    # SOLD TO data row with ▷ for grandchild
    lines.append(
        f'{I}        <tr data-sold-id="{n}-1">'
        f'<td class="expand-icon-cell" onclick="toggleMeisai({n},1)"><span class="expand-icon" id="icon-meisai-{n}-1">▷</span></td>'
    ...
    # Grandchild row: colspan=10 → grandchild-table (INVOICE/明細)
    lines.append(f'{I}        <tr><td colspan="10" style="padding:0;">')
    lines.append(f'{I}          <div class="grandchild-table-container" id="grandchild-meisai-{n}-1">')
